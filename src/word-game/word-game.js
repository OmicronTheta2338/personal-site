/**
 * word-game.js — Word association game logic
 *
 * Manages game modes 1-7, generation, undo, and character destruction
 * callbacks from tank.js explosions.
 */
...
import { ALL_WORDS } from './data/all-words.js';
...
let display, listItems, instructionsContainer, generateBtn;
...
function generateContent() {
    ...
    if (shared.updateColliders) {
        setTimeout(() => shared.updateColliders(), 50);
    }
}
...
export function destroyChar(node, charIndex) {
...
export function initWordGame() {
    display = document.getElementById('wg-display');
    listItems = document.querySelectorAll('#wg-gamemode-list a');
    instructionsContainer = document.getElementById('wg-instructions');
    generateBtn = document.getElementById('wg-generate-btn');
