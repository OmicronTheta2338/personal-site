 ...
 *
 * Uses the full canvas. L-click = reveal / chord, R-click = flag.
 * Mines derive from the CA grid state at activation time.
 */
...
function getHue() {
    return colors.HUE !== undefined ? colors.HUE : 33;
}

function C_FACE() { return "hsl(" + getHue() + ", 41%, 59%)"; }
function C_HI() { return "hsl(" + getHue() + ", 53%, 78%)"; }
function C_SH() { return "hsl(" + getHue() + ", 43%, 33%)"; }
function C_FLAT() { return "hsl(" + getHue() + ", 42%, 71%)"; }
function C_BLACK() { return "hsl(" + getHue() + ", 48%, 7%)"; }
function C_HIT() { return "#b03030"; }
...
imgFlag.src = "/sprites/minesweeper-flag.png";
imgBomb.src = "/sprites/minesweeper-bomb.png";
...
function initBoard(shared) {
    ...
    var caMode = caModes.find(function (m) { return typeof m.getCAState === 'function'; });
...
function drawCell(ctx, c, r, yOffset, showMines) {
    ...
    if (cell.revealed) {
        ...
        ctx.fillRect(x, y, MS, MS);
        ctx.fillStyle = C_HI();
        ctx.fillRect(x, y, MS, B);
        ctx.fillRect(x, y, B, MS);
        ctx.fillStyle = C_SH();
        ctx.fillRect(x, y + MS - B, MS, B);
        ctx.fillRect(x + MS - B, y, B, MS);
